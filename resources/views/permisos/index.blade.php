<x-app-layout>
    ...
    <style>
        ...
        /* Motivo truncado */
        .motivo-cell { max-width:200px; }
        .motivo-text { display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden; font-size:13px; line-height:1.45; color:var(--muted); }

        /* Rango de fechas */
        .date-range { display:flex; flex-direction:column; gap:2px; }
        .date-from { font-size:13px; font-weight:600; color:var(--text); }
        .date-to   { font-size:11px; color:var(--muted); }

        /* Tipo tags */
        .tipo-tag { display:inline-block; padding:2px 8px; border-radius:5px; font-size:11px; font-weight:700; margin-bottom:3px; }
        .tipo-permiso    { background:#f3eef5; color:var(--brand); border:1px solid var(--brand-mid); }
        .tipo-comision   { background:var(--gold-pale); color:var(--brown); border:1px solid var(--gold-light); }
        .tipo-suspension { background:#fce8ee; color:var(--rose); border:1px solid #f0b3c3; }
        .goce-con    { background:#e8f5ee; color:#1b6b38; border:1px solid #b3dfc3; }
        .goce-sin    { background:#fff3cd; color:#856404; border:1px solid #ffe08a; }
        ...
        /* Info semana nominal */
        .week-banner { display:flex; align-items:center; gap:8px; padding:8px 16px; background:var(--brand-light); border-bottom:1px solid var(--border); font-size:12px; color:var(--brand); font-weight:600; }
    ...
    <div class="py-10" style="background-image:url('{{ asset('images/background-pattern.png') }}'); min-height:calc(100vh - 64px);">
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            ...
            <div class="card">
                ...
                {{-- Indicador semana nominal --}}
                @if(!request()->hasAny(['search','estado','tipo','start_date','end_date']))
                    <div class="week-banner">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                        Mostrando semana nominal actual (miércoles–martes)
                    </div>
                @endif
                    ...
                    <div class="filter-bar">
                        ...
                        <div class="filter-group">
                            ...
                            <select name="tipo" class="filter-select">
                                ...
                                <option value="Comisión"   {{ request('tipo') === 'Comisión'   ? 'selected' : '' }}>Comisión</option>
                                <option value="Suspensión" {{ request('tipo') === 'Suspensión' ? 'selected' : '' }}>Suspensión</option>
                            </select>
                        </div>
                        <div class="filter-group">
                                ...
                                <tr>
                                    ...
                                    <td class="hide-mobile">
                                        <div class="date-range">
                                            <span class="date-from">{{ \Carbon\Carbon::parse($solicitud->fecha_inicio)->format('d/m/Y') }}</span>
                                            <span class="date-to">→ {{ \Carbon\Carbon::parse($solicitud->fecha_termino)->format('d/m/Y') }}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="motivo-cell">
                                        <p class="motivo-text" title="{{ $solicitud->motivo }}">{{ $solicitud->motivo }}</p>
                                    </td>
                                    ...
                                    <td style="text-align:right;">
                                        <div style="display:flex; align-items:center; justify-content:flex-end; gap:6px; flex-wrap:wrap;">
                                            <a href="{{ route('permisos.show', $solicitud->id) }}" class="btn btn-gold btn-sm">
                            ...
                            @empty
                                <tr><td colspan="6">
                                    <div class="empty-state">
                                        ...
                                        <a href="{{ route('permisos.index') }}" class="btn btn-outline" style="margin-top:16px;">Limpiar filtros</a>
                                    </div>
                                </td></tr>
                            @endforelse
                        </tbody>
